...
CREATE TABLE IF NOT EXISTS puzzle_solves (
    ...
    -- the time the solve was recorded
    time_taken_to_solve integer CHECK (time_taken_to_solve > 0),
    -- the duration (seconds) of how long it took to solve
    constraint only_one_solve_per_puzzle_and_game UNIQUE(pid, gid)
);
ALTER TABLE public.puzzle_solves OWNER to CURRENT_USER;
