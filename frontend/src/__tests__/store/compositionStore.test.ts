...
describe('compositionStore', () => {
  ...
  describe('attach', () => {
    it('should subscribe to composition events', () => {
      ...
      (onChildAdded as any).mockReturnValue(mockUnsubscribe);
    ...
    it('should emit createEvent when create event is received', () => {
      ...
      let eventCallback: ((snapshot: any) => void) | null = null;
      (onChildAdded as any).mockImplementation((_ref, callback) => {
  ...
  describe('updateCellText', () => {
    it('should push updateCellText event', () => {
      ...
      (push as any).mockReturnValue({key: 'event-1'});
